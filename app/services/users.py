...
from .. import models, users_collection
...
async def get_user(*, id: models.PyObjectId | None = None,
                   username: str | None = None) -> models.User | None:
...
async def create_access_token(user: models.User, expires_delta=timedelta(minutes=15)) -> AccessTokenDict:
    return {
        'token_type': 'bearer',
        'access_token': jwt.encode(key=SECRET_KEY,
                                   algorithm=ALGORITHM,
                                   claims=models.Token(sub=str(user.id),
...
async def get_current_user(token: str = Depends(oauth2_scheme)) -> models.User:
    ...
    try:
        token_data = models.Token.parse_obj(
            jwt.decode(
                token,
                key=SECRET_KEY,
...
async def create_user(user: models.User) -> models.User:
    # TODO: create user
    pass
